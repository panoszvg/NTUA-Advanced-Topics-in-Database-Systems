from pyspark.sql import SparkSession
from time import time
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('-c', '--csv', action='store_true', default=True)
parser.add_argument('-p', '--parquet', action='store_true') # if this option is given, negates csv
args = parser.parse_args()
if args.parquet:
    args.csv = False
    spark = SparkSession.builder.appName("q2-sql").config("spark.sql.parquet.binaryAsString","true").getOrCreate()
    ratings = spark.read.parquet("hdfs://master:9000/user/user/files/ratings.parquet")

if args.csv:
    spark = SparkSession.builder.appName("q2-sql").getOrCreate()
    ratings = spark.read.format("csv").options(headers='false', inferSchema='true').load("hdfs://master:9000/user/user/files/ratings.csv")

ratings.registerTempTable("ratings")

timestamp_1 = time()

res = spark.sql("SELECT (COUNT(Rating)/(SELECT COUNT(*) FROM (SELECT _c0 FROM ratings GROUP BY ratings._c0))) AS Percentage FROM (SELECT AVG(r._c2) AS Rating FROM ratings AS r GROUP BY r._c0 HAVING AVG(r._c2) > 3)")
res.coalesce(1).write.format("com.databricks.spark.csv").mode('overwrite').save("hdfs://master:9000/user/user/outputs/q2-sql.csv")

timestamp_2 = time()
print(timestamp_2 - timestamp_1)
